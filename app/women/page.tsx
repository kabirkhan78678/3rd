...
const WOMEN_CATS = ["All", "Tops", "Hoodies", "Jackets", "Dresses", "Pants", "Accessories"];
const SORT_OPTIONS = [
...
export default function WomenPage() {
  const allWomen = getProductsByCategory("women");
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCategory !== "All") {
      list = list.filter((p) => p.subcategory.includes(activeCategory.toLowerCase().replace(" ", "-")) || p.subcategory === activeCategory.toLowerCase());
    }
    switch (sort) {
    ...
    return list;
  }, [allWomen, activeCategory, sort]);

  return (
    <div style={{ paddingTop: 80 }}>
      {/* Hero Banner */}
      <section style={{ position: "relative", height: 400, overflow: "hidden", display: "flex", alignItems: "center" }}>
        <Image src="/images/women-jacket.jpg" alt="Women's Collection" fill style={{ objectFit: "cover", objectPosition: "center 20%" }} priority sizes="100vw" />
        ...
        <div className="container" style={{ position: "relative", zIndex: 2 }}>
          <motion.div variants={staggerContainer} initial="hidden" animate="visible">
            ...
            <motion.p variants={fadeUp} style={{ fontSize: 15, color: "rgba(255,255,255,0.6)", marginTop: 16 }}>
              {allWomen.length} pieces · New Season
            </motion.p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ position: "relative" }}>
                <select value={sort} onChange={(e) => setSort(e.target.value)}
                  style={{ appearance: "none", padding: "10px 40px 10px 16px", border: "1.5px solid var(--color-border)", background: "transparent", fontSize: 13, fontFamily: "inherit", fontWeight: 600, cursor: "pointer", outline: "none", color: "var(--color-text)" }}>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "80px 0" }}>
              <p style={{ fontSize: 20, fontWeight: 600, marginBottom: 8, color: "var(--color-muted)" }}>No products in this category yet</p>
